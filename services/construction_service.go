...
const (
	// bytesInKB is the number of bytes in a KB. In Thought, this is
...
func (s *ConstructionAPIService) estimateSize(operations []*types.Operation) float64 {
	size := thought.TransactionOverhead
	for _, operation := range operations {
		switch operation.Type {
		case thought.InputOpType:
...
func (s *ConstructionAPIService) ConstructionMetadata(
	...
	if feePerKB < thought.MinFeeRate {
...
func (s *ConstructionAPIService) ConstructionPayloads(
	...
	for _, input := range matches[0].Operations {
		...
		transactionHash, index, err := thought.ParseCoinIdentifier(input.CoinChange.CoinIdentifier)
		if err != nil {
	...
	for i := range tx.TxIn {
		...
		class, _, err := thought.ParseSingleAddress(s.config.Params, script)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionCombine(
	...
	if err := json.Unmarshal(decodedTx, &unsigned); err != nil {
		return nil, wrapErr(
			ErrUnableToParseIntermediateResult,
			fmt.Errorf("%w unable to unmarshal thought transaction", err),
		)
	}
	...
	for i := range tx.TxIn {
		...
		class, _, err := thought.ParseSingleAddress(s.config.Params, decodedScript)
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionHash(
	...
	if err := json.Unmarshal(decodedTx, &signed); err != nil {
		return nil, wrapErr(
			ErrUnableToParseIntermediateResult,
			fmt.Errorf("%w unable to unmarshal signed thought transaction", err),
		)
	}
...
func (s *ConstructionAPIService) parseSignedTransaction(
	...
	for i, input := range tx.TxIn {
		...
		_, addr, err := thought.ParseSingleAddress(s.config.Params, pkScript.Script())
		if err != nil {
			return nil, wrapErr(
				ErrUnableToDecodeAddress,
...
func (s *ConstructionAPIService) ConstructionSubmit(
	...
	if err != nil {
		return nil, wrapErr(ErrThoughtd, fmt.Errorf("%w unable to submit transaction", err))
	}
