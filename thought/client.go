...
import (
	...
	thoughtUtils "github.com/thoughtnetwork/rosetta-thought/utils"
...
const (
	...
	// genesis block of the thought blockchain for polling
	genesisBlockIndex = 0
...
const (
	...
	// returned in Thought blocks to be milliseconds.
	timeMultiplier = 1000

	// rpc credentials are fixed in rosetta-thought
	// because we never expose access to the raw thoughtd
...
// Client is used to fetch blocks from thoughtd and
// to parse Thought block data into Rosetta types.
//
// We opted not to use existing Thought RPC libraries
...
// NewClient creates a new Thought client.
func NewClient(
	baseURL string,
	genesisBlockIdentifier *types.BlockIdentifier,
	currency *types.Currency,
...
// ParseBlock returns a parsed thought block given a raw thought
...
func (b *Client) parseTransactions(
	...
	logger := thoughtUtils.ExtractLogger(ctx, "client")
...
func (b *Client) parseTxOperations(
	...
	for networkIndex, input := range tx.Inputs {
		if thoughtIsCoinbaseInput(input, txIndex, networkIndex) {
...
// `thoughtOutput` transaction output.
func (b *Client) parseOutputTransactionOperation(
	...
	// If we are unable to parse the output account (i.e. thoughtd
...
func (b *Client) getInputTxHash(
	...
	if thoughtIsCoinbaseInput(input, txIndex, inputIndex) {
		return "", -1, false
	}
...
// thoughtIsCoinbaseInput returns whether the specified input is
...
func thoughtIsCoinbaseInput(input *Input, txIndex int, inputIndex int) bool {
...
// parseOutputAccount parses a thoughtScriptPubKey and returns an account
...
// post makes a HTTP request to a Thought node
func (b *Client) post(
	ctx context.Context,
