...
func (msg *MsgCFilter) ThtDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
...
// This is part of the Message interface implementation.
func (msg *MsgCFilter) ThtEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	size := len(msg.Data)
	if size > MaxCFilterDataSize {
		str := fmt.Sprintf("cfilter size too large for message "+
			"[size %v, max %v]", size, MaxCFilterDataSize)
		return messageError("MsgCFilter.ThtEncode", str)
	}
...
// from ThtDecode in that ThtDecode decodes from the bitcoin wire protocol as
