...
func (msg *MsgCFHeaders) ThtDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.ThtDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) ThtEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.ThtEncode", str)
	}
...
// differs from ThtDecode in that ThtDecode decodes from the bitcoin wire
