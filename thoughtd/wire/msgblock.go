...
func (msg *MsgBlock) ThtDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if txCount > maxTxPerBlock {
		...
		return messageError("MsgBlock.ThtDecode", str)
	}

	msg.Transactions = make([]*MsgTx, 0, txCount)
	for i := uint64(0); i < txCount; i++ {
		tx := MsgTx{}
		err := tx.ThtDecode(r, pver, enc)
		if err != nil {
			return err
		}
		msg.Transactions = append(msg.Transactions, &tx)
...
// Version field in the block.  This function differs from ThtDecode in that
// ThtDecode decodes from the bitcoin wire protocol as it was sent across the
...
func (msg *MsgBlock) Deserialize(r io.Reader) error {
	...
	//
	// Passing an encoding type of WitnessEncoding to ThtEncode for the
...
func (msg *MsgBlock) ThtEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
...
// This function differs from ThtEncode in that ThtEncode encodes the block to
