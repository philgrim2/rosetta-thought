...
func (msg *MsgGetData) ThtDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgGetData.ThtDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgGetData) ThtEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgGetData.ThtEncode", str)
	}
